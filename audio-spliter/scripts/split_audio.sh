...
Split audio/video into segments with silence-aware cut points.

Arguments:
  input_file        Input media file (any FFmpeg-compatible format)
...
Output filename format: {source_name}_{num}_[{start}-{end}].{ext}
  Example: video_01_[0-1800].aac, video_02_[1800-3595].aac

Examples:
  $(basename "$0") video.mp4 1800
  $(basename "$0") -w 15 -n "-35dB" video.mp4 1800
  $(basename "$0") --output-dir /tmp podcast.mp3 3600
EOF
    exit 0
}
...
find_silence_point() {
    ...
    local silence_point=$(ffmpeg -i "$input_file" -ss "$window_start" -to "$window_end" \
        -af "silencedetect=noise=$NOISE_THRESHOLD:duration=$SILENCE_DURATION" \
        -f null - 2>&1 | grep "silence_start" | head -1 | sed -n 's/.*silence_start: \([0-9.]*\).*/\1/p')

    if [ -n "$silence_point" ]; then
        echo $(echo "$window_start + $silence_point" | bc)
    else
        echo ""
    fi
}
...
for ((i=0; i<${#cut_points[@]}-1; i++)); do
    ...
    ffmpeg -y -i "$input_file" -ss "$start" -to "$end" -vn -acodec copy "$output_file" 2>/dev/null
done
